...
public class LaneChangeModel {
    ...
    public enum LaneChangeDecision {
        ...
        NONE(Lanes.NO_CHANGE), STAY_IN_LANE(Lanes.NO_CHANGE),
        DISCRETIONARY_TO_LEFT(Lanes.TO_LEFT), DISCRETIONARY_TO_RIGHT(Lanes.TO_RIGHT),
        RANDOM_TO_LEFT(Lanes.TO_LEFT), RANDOM_TO_RIGHT(Lanes.TO_RIGHT),
        MANDATORY_TO_LEFT(Lanes.TO_LEFT), MANDATORY_TO_RIGHT(Lanes.TO_RIGHT), MANDATORY_STAY_IN_LANE(Lanes.NO_CHANGE);
    ...
    public int rightChangeLineProb() {
        if( parameter.getRightChangeLineProb() > 0 ) {
            return 100;
        }
        return parameter.getRightChangeLineProb();
    }

    public int leftChangeLineProb() {
        if( parameter.getLeftChangeLineProb() > 100 ) {
            return 100;
        }
        return parameter.getLeftChangeLineProb();
    }
    ...
    private LaneChangeDecision determineDiscretionaryLaneChangeDirection(RoadSegment roadSegment) {
        ...
        // decision
        if ((accToRight > 0 && checkProbability(this.rightChangeLineProb())) || (accToLeft > 0 && checkProbability(this.leftChangeLineProb()))) {
            LOG.debug("accToRight={}, accToLeft={}", accToRight, accToLeft);
            LOG.debug("currentLane={}", currentLane);
            if (accToRight > accToLeft) {
                return LaneChangeDecision.RANDOM_TO_RIGHT;
            }
            return LaneChangeDecision.RANDOM_TO_LEFT;
        }
    ...
    private boolean checkProbability( int val) {
